...
public class RouteAdapter extends ArrayAdapter<Route> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (v == null) {
			...
			v = inflater.inflate(R.layout.route_results_row, null);
		...
		String departureTime = KlmobiUtils.time_format.format(route.routeComponents.get(0).startDateTime);
		holder.departureView.setText(" " + departureTime);
		
		String arrivalTime = KlmobiUtils.time_format.format(route.routeComponents.get(route.routeComponents.size()-1).endDateTime);
		holder.arrivalView.setText(arrivalTime + " ");

		long duration = Math.round(route.duration);
		holder.durationView.setText((duration > 59 ? Long.toString(duration / 60) + " h " : "") + Long.toString(duration % 60) + " min");
		...
		for (int i = 0, len = route.routeComponents.size(); i < len; ++i) {
			...
			if ("W".equals(routeComp.code)) { // Walking
				ImageView walkIcon = new ImageView(this.context);
				...
				LinearLayout busIconLayout = new LinearLayout(this.context);
				...
				ImageView busIcon = new ImageView(this.context);
				busIcon.setImageResource(R.drawable.ic_bus);
				busIconLayout.addView(busIcon);
				
				TextView lineNumber = new TextView(this.context);
		...
		String firstStopDepartureTime = (firstBusIndex == -1) ? "-" : KlmobiUtils.time_format.format(route.routeComponents.get(firstBusIndex).startDateTime);
		holder.stopDepView.setText("(" + firstStopDepartureTime + ")");
		
		String lastStopArrivalTime = (lastBusIndex == -1) ? "-" : KlmobiUtils.time_format.format(route.routeComponents.get(lastBusIndex).endDateTime);
